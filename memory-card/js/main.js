...
function shuffle(n) {
    for (var e, i, a = n.length; 0 !== a;) i = Math.floor(Math.random() * a), a -= 1, e = n[a], n[a] = n[i], n[i] = e;
    return n
}

function playSound(n) {
    document.getElementById(n + "-sound").load(), document.getElementById(n + "-sound").play()
}

function openModal(n) {
    $(".modal-backdrop").css("display", "block"), $(".modal").hide(), $(".modal." + n).fadeIn()
}

function closeModal() {
    $(".modal-backdrop").css("display", "none"), $(".modal").hide()
}
...
function startGame(n) {
    1 == n ? (maxTime = remainingTime = normalTime, $("#bg-music").children().first().attr("src", "audio/normal.mp3"), $(".modal.win img").first().attr("src", "img/win1.png")) : (maxTime = remainingTime = hardTime, $("#bg-music").children().first().attr("src", "audio/hard.mp3"), $(".modal.win img").first().attr("src", "img/win2.png")), closeModal(), $(".btn-reset").css("opacity", "0"), isPlaying = !0, point = 0;
    $(".card").css("pointer-events", "auto"), document.getElementById("bg-music").load(), document.getElementById("bg-music").play(), remainingTime = maxTime, $(".progressbar").css("display", "block"), $("progress").val(100), running = setInterval(function() {
        remainingTime--, $("progress").val(remainingTime / maxTime * 100), 10 == remainingTime && playSound("count10"), 5 == remainingTime && playSound("count5"), 0 == remainingTime && (document.getElementById("bg-music").load(), $("#sound").children().first().attr("src", "audio/lose.mp3"), playSound("lose"), stopGame(), openModal("lose"), $(".btn-reset").css("opacity", "1"))
    }, 1e3)
}

function stopGame() {
    isPlaying = !1, null != running && (clearInterval(running), running = null), $(".card").css("pointer-events", "none")
}
var isPlaying = !1,
    current = null,
    cards = ["f1", "f2", "f3", "f4", "f5", "f6", "f7", "f8", "f9", "f10", "f11", "f12"],
    numberCards = 12,
    point = 0,
    normalTime = 75,
    hardTime = 62,
    ...
    running = null;
$(function() {
    document.getElementById("flip-sound").volume = .4, document.getElementById("incorrect-sound").volume = .4, document.getElementById("correct-sound").volume = .4, cards = cards.concat(cards);
    var n = Math.floor(3 * Math.random() + 1);
    $("body").css("background", 'url("img/bg' + n + '.jpg") center center no-repeat'), loadContent()
});